...
//QVector<AudioMixer*> AudioMixer::instances_;
...
void AudioMixer::SetParams(const AudioParams &params) {
  if (params_ != params) {
    params_ = params;
...
void AudioMixer::PushBytes(const QByteArray &d) {
...
void AudioMixer::StartWaveform(const AudioVisualWaveform *waveform, const rational &start, int playback_speed) {
  Stop();
...
void AudioMixer::SetUpdateLoop(bool e) {
  if (e) {
    connect(this, &AudioMixer::frameSwapped, this, static_cast<void (AudioMixer::*)()>(&AudioMixer::update));

    update();
  } else {
    disconnect(this, &AudioMixer::frameSwapped, this, static_cast<void (AudioMixer::*)()>(&AudioMixer::update));
  }
}

void AudioMixer::paintGL() {
  QPainter p(this);
...
void AudioMixer::mousePressEvent(QMouseEvent *)
{
  peaked_.fill(false);
  update();
}

void AudioMixer::UpdateValuesFromFile(QVector<double>& v, qint64 delta_time)
{
...
void AudioMixer::UpdateValuesFromWaveform(QVector<double> &v, qint64 delta_time)
{
...
void AudioMixer::PushValue(const QVector<double> &v)
{
...
void AudioMixer::BytesToSampleSummary(const QByteArray &b, QVector<double> &v)
{
...
QVector<double> AudioMixer::GetAverages() const
{
  QVector<double> v(params_.channel_count(), 0);
